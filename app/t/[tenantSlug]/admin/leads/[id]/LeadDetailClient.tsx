...
export default function LeadDetailClient({
  ...
  activeStages: string[]
  initialCoAssignment: CoAssignment
}) {
  ...
  // Co-assignment state
  const [coAssignment, setCoAssignment] = useState<CoAssignment>(initialCoAssignment)
  ...
  useEffect(() => {
    ; (async () => {
      try {
        ...
        const rows = (data?.data?.stages ?? data?.stages ?? []) as Array<{ key: string; label: string; meta?: { assignmentTrigger?: boolean } | null }>
  ...
  // Whether the current primary stage requires co-assignment
  const isCoAssignTriggerStage = pipelineStages.some(
    (s) => s.key === primaryStage && s.meta?.assignmentTrigger === true,
  )
  ...
  const handleSaveCoAssign = async () => {
    ...
    const data = await apiCall(async () => {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error ?? 'Could not co-assign lead')
      return res.json()
    }, { successMsg: 'Lead shared successfully', errorMsg: 'Could not co-assign lead' })
    setSavingCoAssign(false)
    if (!data) return
    // Refresh co-assignment display
  ...
  const handleRemoveCoAssign = async () => {
    ...
    const data = await apiCall(async () => {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error ?? 'Could not remove co-assignment')
      return res.json()
          ...
          <div className={`bg-white border border-slate-200 rounded-xl p-5 shadow-crm-sm dark:bg-[#0f172a] dark:border-slate-700 ${isDeadState ? 'pointer-events-none opacity-50' : ''}`}>
            ...
            {stagesLoading ? (
              ...
              <>
                ...
                {/* Co-assign Panel — visible only when current stage is an assignment trigger */}
                    ...
                    {coAssignment ? (
                      <div className="space-y-3">
                        ...
                        <button
                          ...
                          className="w-full h-8 text-xs font-medium border border-slate-200 text-slate-500 hover:text-red-500 hover:border-red-200 rounded-lg transition-colors disabled:opacity-40 flex items-center justify-center gap-1.5 dark:border-slate-700 dark:text-slate-400"
                        >
                      ...
                      <div className="flex flex-col gap-2">
                        <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">
                          This stage requires a co-assignee. Select a team member to share full access with.
                        </p>
                        <Select
                          value={selectedCoAssignee || 'none'}
                          disabled={isDeadState || savingCoAssign}
                          onValueChange={(val) => setSelectedCoAssignee(val === 'none' ? '' : val)}
                        >
                          <SelectTrigger className="w-full h-9">
